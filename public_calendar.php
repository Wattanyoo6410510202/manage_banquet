    ...
    return new FullCalendar.Calendar(document.getElementById('calendar'), Object.assign({}, calOpts(), {
        ...
        events: <?php
            ...
            $q = $conn->query("SELECT f.*, r.room_name, c.cust_name, c.cust_phone, comp.company_name, ft.type_name
                ...
                WHERE f.status!='Cancelled' ORDER BY f.id ASC");
            ...
            $q2=$conn->query("SELECT q.*,c.cust_name,c.cust_phone FROM quotations q LEFT JOIN customers c ON q.customer_id=c.id WHERE q.status NOT IN ('Cancelled') ORDER BY q.id DESC");
