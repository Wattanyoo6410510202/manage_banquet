...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['add'])) {
        ...
        $conn->query("INSERT INTO master_checklist_mt (task_detail) VALUES ('$name')");
        $msg = "เพิ่มรายการเรียบร้อย";
    } elseif (isset($_POST['delete'])) {
        $id = intval($_POST['id']);
        $conn->query("DELETE FROM master_checklist_mt WHERE id = $id");
        $msg = "ลบรายการเรียบร้อย";
    }
}

$items = $conn->query("SELECT * FROM master_checklist_mt ORDER BY id ASC");
?>
<div class="container-fluid">
    <h4 class="mb-4"><i class="bi bi-tools"></i> จัดการรายการตรวจสอบ (แผนกช่าง)</h4>
