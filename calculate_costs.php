...
$sum_main = 0;
$sum_main_cost = 0;
$sql_m = "SELECT menu_detail, menu_qty, menu_price, menu_cost FROM function_menus WHERE function_id = $id";
...
$sum_break = 0;
$sum_break_cost = 0;
$sql_k = "SELECT k_item, k_qty, k_price, k_cost FROM function_kitchens WHERE function_id = $id";
...
while ($row = $res_k->fetch_assoc()) {
    ...
    if ($price > 0) {
        ...
        $cost_total = $cost > 0 ? $cost * (float)$row['k_qty'] : $total;
        $break_list[] = ['name' => $row['k_item'], 'qty' => (float)$row['k_qty'], 'price' => $price, 'cost' => $cost > 0 ? $cost : $price, 'total' => $total, 'cost_total' => $cost_total];
        $sum_break += $total;
