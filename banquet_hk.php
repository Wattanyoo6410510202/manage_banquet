                ...
                <div class="card mb-3 border-0 shadow-sm" style="border-radius: 12px;">
                    <div class="card-body p-3">
                        ...
                        <div class="bg-light p-2 rounded mb-2 small">
                            <strong class="d-block mb-1">การตกแต่งและดูแล (แม่บ้าน):</strong>
                            <strong>ฉากหลัง:</strong> <?= htmlspecialchars($row['backdrop_detail'] ?: '-'); ?><br>
                            <strong>ดอกไม้/ความสะอาด:</strong> <?= htmlspecialchars($row['hk_florist_detail'] ?: '-'); ?>
                        </div>
...
<script>
    function showEventDetail(title, functionId) {
        ...
        // บังคับให้เป็น role housekeeping เพื่อให้แสดงเฉพาะส่วนที่เกี่ยวข้อง
    ...
    function loadEventInfoModal(functionId, userRole) {
        fetch(`api/get_modal_data.php?id=${functionId}`)
            ...
            .then(response => {
                ...
                ['hk_textarea'].forEach(textareaId => {
            ...
            .then(html => {
                ...
                if (filteredRows) {
                    document.getElementById('financeContainer').innerHTML = `
                        <h6 class="fw-bold mb-3">รายการการเงิน (แผนกแม่บ้าน)</h6>
                        <div class="table-responsive">
                            <table class="table table-hover align-middle">
                                <thead class="table-light"><tr>${doc.querySelector('thead').innerHTML}</tr></thead>
                                <tbody class="bg-white">${filteredRows}</tbody>
                            </table>
                        </div>
                    `;
                }
